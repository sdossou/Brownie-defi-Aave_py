...
def main():
    ...
    # DAI converted from ETH?
    dai_eth_price = get_asset_price(
        config["networks"][network.show_active()]["dai_eth_price_feed"]
    )
    amount_dai_to_borrow = (borrowable_eth * 0.95) * (1 / dai_eth_price)
    # borrowable_eth -> borrowable_dai * 95% as a buffer so we don't get liquidated
...
def get_asset_price(price_feed_address):
    ...
    print(f"The DAI/ETH latest price is {Web3.fromWei(latest_price, "ether")}")
